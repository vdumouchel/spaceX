...
var _default = new _graphql.GraphQLScalarType({
  ...
  serialize: function serialize(value) {
    ...
    if (!PHONE_NUMBER_REGEX.test(value)) {
      throw new TypeError("Value is not a valid phone number of the form +17895551234 (10-15 digits): ".concat(value));
    }
  ...
  parseLiteral: function parseLiteral(ast) {
    if (ast.kind !== _language.Kind.STRING) {
      throw new _error.GraphQLError("Can only validate strings as phone numbers but got a: ".concat(ast.kind));
    }

    if (!PHONE_NUMBER_REGEX.test(ast.value)) {
      throw new TypeError("Value is not a valid phone number of the form +17895551234 (10-15 digits): ".concat(ast.value));
    }
